...
describe('Acesso a área de cadastro de novos usuários.', () => {
    beforeEach(() => {
        ...
        it('Cadastro de novo usuário formato Senha Invalida', () => {
            cy.get('[data-test="input-name"]').type('John Doe');
            cy.get('[data-test="input-email"]').type('johndoe@email.com');
            cy.get('[data-test="input-password"]').type('Senha123');
            cy.get('[data-test="input-confirm-password"]').type('123456');
            cy.get('[data-test="submit-button"]').click();
            cy.get('.error').should('have.text','As senhas não batem');
